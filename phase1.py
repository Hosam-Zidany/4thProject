...
feature_importance = pd.DataFrame({"Symptom": X.columns, "Weight": model.coef_[0]})

# Symptoms most associated with anxiety
top_positive = feature_importance.sort_values(by="Weight", ascending=False).head(10)

top_positive.set_index("Symptom")["Weight"].plot(kind="barh")
plt.title("Top Symptoms Associated with Anxiety")
plt.show()

# Symptoms most associated with NOT anxiety
top_negative = feature_importance.sort_values(by="Weight").head(10)
top_negative.set_index("Symptom")["Weight"].plot(kind="barh", color="red")
plt.title("Top Symptoms Associated with Anxiety")
plt.show()

print("\nTop symptoms indicating Anxiety:")
print(top_positive)

print("\nSymptoms indicating NOT Anxiety:")
print(top_negative)
